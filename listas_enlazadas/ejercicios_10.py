...
# Eliminar las canciones del artista que menos tiene
    ...
    def _generate_shuffle_list(self): 
        ...
        while current: # Recorre la lista
            self.shuffle_list.append(current) # Agrega cada nodo a la lista
            current = current.next # Avanza
        random.shuffle(self.shuffle_list) # Reordena aleatoriamente
        self.shuffle_list = self.shuffle_list.copy() # Copia por seguridad
        self.current_shuffle_idx = 0 # Índice actual de reproducción aleatoria
        print(self.shuffle_list) # Muestra la lista (opcional)

    # Maneja la navegación en modo shuffle
    def _handle_shuffle_next(self): 
        if self.current_shuffle_idx < len(self.shuffle_list) - 1: # Si no llegó al final
            self.current_shuffle_idx += 1
        else:
            self.current_shuffle_idx = 0   # Vuelve al inicio si terminó
